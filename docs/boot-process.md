# Processo de boot — UTAMO OS v0.2
...
Não se usa terminal Limine, identity mapping improvisado, APs ou ring 3.
...
11. Remapeia PIC com fontes mascaradas, configura PIT e PS/2, libera somente
    IRQ0/IRQ1 e habilita IF.
12. Emite `UTAMO OS ready.`, inicializa shell e mostra `utamo> `.
13. Processa input fora da ISR; antes de repousar, CLI, nova consulta à fila
    e STI/HLT contíguos fecham a janela de perda de wakeup.
...
Não há reclaim de BOOTLOADER_RECLAIMABLE/ACPI nem troca de CR3 para uma
nova raiz. [Gerenciamento de memória](memory-management.md) descreve os checks.
...
| Exceção após IDT | Dump completo serial, diagnóstico opcional e tentativa de framebuffer; halt |
...
recuperação de page fault, scheduler ou shutdown ACPI.
...
automaticamente uma nova imagem v0.2.
